...
def medir_desempeno_ordenamiento(dataset_unificado):
    ...
    con el nombre, tamano del dataset y tiempo de ejecucion.
    """
    algoritmos = [
    ...
    for nombre, func in algoritmos:
        ...
        try:
            ...
            print(f"[OK] {nombre} finalizado en {tiempo_total:.6f} segundos.")
            ...
            print(f"[ERROR] {nombre} fallo: {type(exc).__name__}: {exc}")
...
def generar_grafico_barras_tiempos(resultados_ordenados, ruta_salida):
    ...
    try:
        ...
        print(
            "[ADVERTENCIA] No se pudo generar el grafico: falta matplotlib. "
    ...
    if not validos:
        print("[ADVERTENCIA] No hay tiempos validos para graficar.")
        return False
